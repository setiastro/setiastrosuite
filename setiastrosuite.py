...
class BlinkTab(QWidget):
    ...
    def on_right_click(self, pos):
        ...
        if item:
            ...
            move_action.triggered.connect(lambda: self.move_item(item))
    ...
    def move_item(self, item):
        """Allow the user to move the selected image to a different directory."""
        current_name = item.text(0)
        file_path = next((path for path in self.image_paths if os.path.basename(path) == current_name), None)

        if file_path:
            # Open file dialog to select a new directory
            new_directory = QFileDialog.getExistingDirectory(self, "Select Destination Folder", os.path.dirname(file_path))
            ...
            if new_directory:
                ...
                try:
                    ...
                    # Update the image paths and tree view
                    self.image_paths[self.image_paths.index(file_path)] = new_file_path
                    item.setText(0, current_name)  # Update the tree view item with the same name (if it remains the same)
                except Exception as e:
