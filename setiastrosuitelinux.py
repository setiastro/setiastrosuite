...
class CosmicClarityTab(QWidget):
    ...
    def run_cosmic_clarity(self, input_file_path=None):
        ...
        # Determine mode and output filename suffix
        mode = "sharpen"
        output_suffix = "_sharpened"
        if self.denoise_radio.isChecked():  # Switch to "denoise" if the radio button is selected
            mode = "denoise"
        ...
        batch_script_path = self.create_batch_script(batch_file_path, mode)
    ...
    def run_cosmic_clarity_on_cropped(self, cropped_image, apply_autostretch=False):
        ...
        # Determine the mode and output filename suffix
        mode = "sharpen"
        output_suffix = "_sharpened"
        if self.denoise_radio.isChecked():  # Switch to "denoise" if the radio button is selected
            mode = "denoise"
    ...
    def create_batch_script(self, batch_file_path, mode):
        """Generate the batch or shell script to run Cosmic Clarity."""
        print(f"Creating batch script for mode: {mode}")  # Debug print
        # Ensure correct file extension based on the operating system
        if os.name == 'nt':  # Windows
            batch_file_path += ".bat"
            exe_path = os.path.join(
                self.cosmic_clarity_folder,
                "SetiAstroCosmicClarity.exe" if mode == "sharpen" else "SetiAstroCosmicClarity_denoise.exe"
            )
            ...
            batch_file_path += ".sh"
            exe_name = "SetiAstroCosmicClarity" if mode == "sharpen" else "SetiAstroCosmicClarity_denoise"
